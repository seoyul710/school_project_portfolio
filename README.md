...
- 파일 존재 여부, `.pdf` 확장자, PDF header, 최대 20MB 크기 검사
...
웹과 CLI는 동일한 검증·저장 코드를 사용합니다. 제목·날짜·태그·URL도 검사하고, 동시 등록은 잠금으로 제한합니다. 임시 파일에 준비한 다음 기존 파일을 덮어쓰지 않고 저장하며, 정상적인 오류 발생 시 새 파일을 정리하고 기존 데이터를 유지합니다. PDF 처리는 별도 Worker에서 40초 제한으로 실행합니다.
